    ...
    if (empty($ime) || empty($priimek) || empty($uporabnisko_ime) || empty($email) || empty($geslo)) {
        ...
        try {
            ...
            if ($stmt->fetch()) {
                ...
                $stmt = $pdo->prepare("INSERT INTO uporabniki (ime, priimek, uporabnisko_ime, email, geslo, vloga, datum_registracije, datum_rojstva, status) VALUES (?, ?, ?, ?, ?, 'ucenec', NOW(), ?, 'aktiven')");
                $stmt->execute([$ime, $priimek, $uporabnisko_ime, $email, $geslo_hash, $datum_rojstva ?: null]);
                $sporocilo = 'Učenec uspešno dodan.';
...
<body>
    <div class="content-container">
        <div class="form-box">
            ...
            <form method="POST" action="dodaj_ucenca.php">

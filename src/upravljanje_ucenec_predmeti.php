...
$id_ucenca = isset($_GET['id_ucenca']) ? (int)$_GET['id_ucenca'] : 0;
if ($id_ucenca <= 0) {
    header('Location: upravljanje_ucenci.php');
    exit;
}

// Pridobi podatke o učencu
$stmt = $pdo->prepare("SELECT id, ime, priimek FROM uporabniki WHERE id = ? AND vloga = 'ucenec'");
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['dodaj_predmet'])) {
    ...
    if ($id_predmeta > 0) {
        try {
            ...
            $stmt = $pdo->prepare("SELECT id FROM ucenci_predmeti WHERE id_ucenca = ? AND id_predmeta = ?");
            $stmt->execute([$id_ucenca, $id_predmeta]);
            if ($stmt->fetch()) {
                $sporocilo = 'Učenec že obiskuje ta predmet.';
            } else {
...
if (isset($_GET['opusci_predmet']) && is_numeric($_GET['opusci_predmet'])) {
    $id_predmeta = (int)$_GET['opusci_predmet'];
    try {
...
if (isset($_GET['vpisi_predmet']) && is_numeric($_GET['vpisi_predmet'])) {
    $id_predmeta = (int)$_GET['vpisi_predmet'];
    try {
        $stmt = $pdo->prepare("UPDATE ucenci_predmeti SET status = 'vpisano', datum_vpisa = CURDATE() WHERE id_ucenca = ? AND id_predmeta = ?");
        $stmt->execute([$id_ucenca, $id_predmeta]);
        $sporocilo = 'Učenec uspešno vpisan v predmet.';
    } catch (PDOException $e) {
...
<body>
    <div class="content-container">
        <div class="table-box">
            <h3>Predmeti učenca: <?php echo htmlspecialchars($ucenec['ime'] . ' ' . $ucenec['priimek']); ?></h3>
            ...
            <table>
                ...
                <tbody>
                    <?php if (empty($predmeti_ucenca)): ?>
                        <tr>
                            <td colspan="5" style="text-align: center;">Učenec še ne obiskuje nobenega predmeta.</td>
                        </tr>
                    <?php else: ?>
                        <?php foreach ($predmeti_ucenca as $predmet): ?>
                            <tr>
                                ...
                                <td>
                                    <?php if ($predmet['status'] === 'vpisano'): ?>
                                        <button class="btn-action btn-opusci" onclick="if(confirm('Ali ste prepričani, da želite opustiti ta predmet?')) window.location.href='?id_ucenca=<?php echo $id_ucenca; ?>&opusci_predmet=<?php echo $predmet['id']; ?>'">Opusti</button>
                                    <?php else: ?>
                                        <button class="btn-action btn-vpisi" onclick="window.location.href='?id_ucenca=<?php echo $id_ucenca; ?>&vpisi_predmet=<?php echo $predmet['id']; ?>'">Vpiši</button>
                                    <?php endif; ?>
                                </td>
                            </tr>
                        <?php endforeach; ?>
            ...
            <div class="form-add">
                <h4>Dodaj predmet:</h4>
                <form method="POST" action="upravljanje_ucenec_predmeti.php?id_ucenca=<?php echo $id_ucenca; ?>">
                    <select name="id_predmeta" required>
                        <option value="">-- Izberi predmet --</option>
                        <?php foreach ($vsi_predmeti as $predmet): ?>
                            <?php 
                            $ze_dodan = false;
                            foreach ($predmeti_ucenca as $p) {
                                if ($p['id'] == $predmet['id']) {
                                    $ze_dodan = true;
                                    break;
                                }
            ...
            <div style="text-align: center; margin-top: 20px;">
                <a href="upravljanje_ucenci.php" style="color: var(--color-text); text-decoration: none;">⬅ Nazaj na seznam učencev</a>
            </div>
        </div>
    </div>
</body>

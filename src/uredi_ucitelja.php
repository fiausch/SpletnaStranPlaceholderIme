...
$id_ucitelja = isset($_GET['id']) ? (int)$_GET['id'] : 0;
if ($id_ucitelja <= 0) {
...
$stmt = $pdo->prepare("SELECT id, ime, priimek, uporabnisko_ime, email, datum_rojstva, status FROM uporabniki WHERE id = ? AND vloga = 'ucitelj'");
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($ime) || empty($priimek) || empty($uporabnisko_ime) || empty($email)) {
        ...
        try {
            // Preveri, če email ali uporabniško ime že obstaja pri drugem uporabniku
            $stmt = $pdo->prepare("SELECT id FROM uporabniki WHERE (email = ? OR uporabnisko_ime = ?) AND id != ?");
            $stmt->execute([$email, $uporabnisko_ime, $id_ucitelja]);
            if ($stmt->fetch()) {
                ...
                if (!empty($geslo)) {
                    ...
                    $stmt->execute([$ime, $priimek, $uporabnisko_ime, $email, $datum_rojstva ?: null, $status, $geslo_hash, $id_ucitelja]);
                } else {
                    $stmt = $pdo->prepare("UPDATE uporabniki SET ime = ?, priimek = ?, uporabnisko_ime = ?, email = ?, datum_rojstva = ?, status = ? WHERE id = ?");
                    $stmt->execute([$ime, $priimek, $uporabnisko_ime, $email, $datum_rojstva ?: null, $status, $id_ucitelja]);
                }
                $sporocilo = 'Učitelj uspešno posodobljen.';
                // Osveži podatke
...
<body>
    <div class="content-container">
        <div class="form-box">
            ...
            <form method="POST" action="uredi_ucitelja.php?id=<?php echo $id_ucitelja; ?>">
                <label for="ime">Ime *</label>
                <input type="text" id="ime" name="ime" value="<?php echo htmlspecialchars($ucitelj['ime']); ?>" required>
                
                <label for="priimek">Priimek *</label>
                <input type="text" id="priimek" name="priimek" value="<?php echo htmlspecialchars($ucitelj['priimek']); ?>" required>
                
                <label for="uporabnisko_ime">Uporabniško ime *</label>
                <input type="text" id="uporabnisko_ime" name="uporabnisko_ime" value="<?php echo htmlspecialchars($ucitelj['uporabnisko_ime']); ?>" required>
                
                <label for="email">E-mail *</label>
                <input type="email" id="email" name="email" value="<?php echo htmlspecialchars($ucitelj['email']); ?>" required>
                ...
                <input type="date" id="datum_rojstva" name="datum_rojstva" value="<?php echo $ucitelj['datum_rojstva'] ? htmlspecialchars($ucitelj['datum_rojstva']) : ''; ?>">
                
                <label for="status">Status *</label>
                <select id="status" name="status" required>
                    <option value="aktiven" <?php echo ($ucitelj['status'] === 'aktiven') ? 'selected' : ''; ?>>Aktiven</option>
                    <option value="neaktiven" <?php echo ($ucitelj['status'] === 'neaktiven') ? 'selected' : ''; ?>>Neaktiven</option>
                </select>
